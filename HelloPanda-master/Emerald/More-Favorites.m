...
#import "Config.h"
#import "Tools.h"
@interface More_Favorites ()

@end

@implementation More_Favorites
...
- (void)viewDidLoad
{
    ...
    lblTitleNav.text = self.stringTitle;
    
    
}
-(void)ShouldDisplayFavErrorView:(BOOL)val{
    if (viewFavError==nil) {
        viewFavError = [[UIView alloc]init];
        viewFavError.backgroundColor=[UIColor clearColor];
        UIImageView *viewImg = [[UIImageView alloc]init];
        viewImg.backgroundColor=[UIColor clearColor];
        if([UIScreen mainScreen].bounds.size.height == 568){
            viewFavError.frame = CGRectMake(0, viewHeader.frame.size.height, tableV.frame.size.width, 473);
            [viewImg setImage:[UIImage imageNamed:@"NoFavErrori5.png"]];
        }
        else
        {
            viewFavError.frame = CGRectMake(0, viewHeader.frame.size.height, tableV.frame.size.width, 385);
            [viewImg setImage:[UIImage imageNamed:@"NoFavError.png"]];
        }
        viewImg.frame = CGRectMake(0, 0, viewFavError.frame.size.width, viewFavError.frame.size.height);
        [viewFavError addSubview:viewImg];
        UIButton *btnAction = [[UIButton alloc]initWithFrame:viewImg.frame];
        [viewFavError addSubview:btnAction];
        [self.view addSubview:viewFavError];
        viewFavError.hidden=YES;
    }
    if (val && dataTable.count==0) {
        viewFavError.hidden=NO;
        [self.view bringSubviewToFront:viewFavError];
    }
    else
    {
        viewFavError.hidden=YES;
...
- (void)RefreshInfos{
    dataTable = nil;
    dataTable = [LocalData getFavArticles];
    ...
    if (dataTable.count==0) {
        [self ShouldDisplayFavErrorView:YES];
    }
    else
    {
        [self ShouldDisplayFavErrorView:NO];
    }
}
