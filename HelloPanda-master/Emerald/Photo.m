...
- (void)viewDidLoad
{
    ...
    tableV.hidden=YES;
    feedLoader = [[PhotoDataManager alloc]init];
...
-(void)viewWillAppear:(BOOL)animated{
    if ([Config isPhotoRibbonEnabled]) {
        [tableV reloadData];
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	if (cell == nil) {
        ...
        //Background
        UIButton *background = [[UIButton alloc]initWithFrame:CGRectMake((self.view.frame.size.width-294)/2, ([Config getPhotoCellSize]-201)/2, 294, 201)];
        ...
        lblTitle.textColor = [Config getPhotoTitleColor];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return [Config getPhotoCellSize];
}
...
-(void)UserTouchedShare:(id)sender{
    if ([sender tag]-1>=0 && [sender tag]-1<[dataTable count]) {
        ...
        [sharing ShowShareToolsInController:self withMessage:[NSString stringWithFormat:@"%@ %@",[Config getPhotoSharingMessage],[[dataTable objectAtIndex:[sender tag]-1]objectForKey:@"title"]] andUrl:[[dataTable objectAtIndex:[sender tag]-1]objectForKey:@"link"]];
        
    }
}
-(void)UserTouchedImage:(id)sender{
    if ([sender tag]-1<[dataTable count]) {
        FullScreenPhoto *fl = [[FullScreenPhoto alloc]init];
